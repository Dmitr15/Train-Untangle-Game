...
public class Train  extends abstractPlatform{
    ...
    // Проверка возможности перемещения по путям
        if (paths.contains(nextPosition)) {
            ...
            if (!field.ifPlatformExists() || field.ifPlatformExists() && !(field.getPlatforms() instanceof Platform)) {
                position = nextPosition;
            } else if (field.ifPlatformExists() && field.getPlatforms() instanceof Platform && ((Platform) field.getPlatforms()).isMoveable()) {
                position = nextPosition;
            }
            else if (field.ifPlatformExists() && field.getPlatforms() instanceof Platform && !((Platform) field.getPlatforms()).isMoveable()) {
